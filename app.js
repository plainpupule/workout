...
const state=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null')||{week:1,travel:false,logs:{},bodyLogs:[],profile:{startDate:new Date().toISOString().slice(0,10)},settings:{restSeconds:90,units:'lb'}};
...
function closeTimer(){clearInterval(timerInterval);document.getElementById('timerDock').hidden=true}
function numeric(v){const n=parseFloat(String(v??'').replace(/[^0-9.\-]/g,''));return Number.isFinite(n)?n:null}
function targetTop(reps){const nums=String(reps).match(/\d+/g)||[];return nums.length?Number(nums.at(-1)):10}
function previousExerciseLog(dayId,index){if(state.week<=1)return null;return state.logs[`w${state.week-1}-${dayId}`]?.exercises?.[index]||null}
function progressionSuggestion(day,index){const prior=previousExerciseLog(day.id,index);const priorSession=state.week>1?state.logs[`w${state.week-1}-${day.id}`]:null;if(!prior?.sets?.length)return 'Start conservatively and leave the planned reps in reserve.';const rows=prior.sets.filter(x=>numeric(x.reps)!==null);if(!rows.length)return 'Use last week as a technique baseline and record every working set.';if(Number(priorSession?.pain)>=4)return 'Hand pain was elevated last time—repeat or reduce load and prioritize control.';const top=targetTop(weekScheme[Math.max(1,state.week-1)].reps);const hitTop=rows.every(x=>numeric(x.reps)>=top);const weights=rows.map(x=>numeric(x.weight)).filter(x=>x!==null);if(hitTop&&weights.length){const best=Math.max(...weights);const bump=Math.max(2.5,Math.round(best*.04*2)/2);return `All sets reached the top of the range. Try about ${best+bump} ${state.settings.units}, only if form and hand comfort stay solid.`}return 'Keep the same load and add one rep to one or more sets before increasing weight.'}
function workoutVolume(log){let total=0;Object.values(log?.exercises||{}).forEach(ex=>(ex.sets||[]).forEach(s=>{const w=numeric(s.weight),r=numeric(s.reps);if(w!==null&&r!==null)total+=w*r}));return Math.round(total)}
function svgLineChart(values,label){if(!values.length)return '<p class="muted">Complete workouts to start this chart.</p>';const w=640,h=190,pad=24,max=Math.max(...values.map(x=>x.value),1),min=Math.min(...values.map(x=>x.value),0),span=Math.max(max-min,1);const pts=values.map((x,i)=>`${pad+(i*(w-pad*2)/Math.max(values.length-1,1))},${h-pad-((x.value-min)/span)*(h-pad*2)}`).join(' ');return `<div class="svg-chart"><svg viewBox="0 0 ${w} ${h}" role="img" aria-label="${label}"><defs><linearGradient id="chartG" x1="0" y1="0" x2="1" y2="0"><stop stop-color="#b7ff4a"/><stop offset="1" stop-color="#7c5cff"/></linearGradient></defs><path d="M${pad} ${h-pad}H${w-pad}" stroke="rgba(255,255,255,.12)"/><polyline points="${pts}" fill="none" stroke="url(#chartG)" stroke-width="6" stroke-linecap="round" stroke-linejoin="round"/>${values.map((x,i)=>{const [cx,cy]=pts.split(' ')[i].split(',');return `<circle cx="${cx}" cy="${cy}" r="6" fill="#0e1220" stroke="#b7ff4a" stroke-width="4"><title>${x.label}: ${x.value}</title></circle>`}).join('')}</svg></div>`}
...
function renderProgress(){
 const p=document.getElementById('progress'); const completedLogs=Object.entries(state.logs).filter(([,x])=>x.completed).sort((a,b)=>new Date(a[1].date||0)-new Date(b[1].date||0)); const completed=completedLogs.length;
 const mobility=completedLogs.map(([k,x])=>({label:k,value:Number(x.mobility)})).filter(x=>Number.isFinite(x.value)&&x.value>0).slice(-12);
 const pain=completedLogs.map(([k,x])=>({label:k,value:Number(x.pain)})).filter(x=>Number.isFinite(x.value)).slice(-12);
 const volumes=completedLogs.map(([k,x])=>({label:k,value:workoutVolume(x)})).filter(x=>x.value>0).slice(-12);
 const consistency=[1,2,3,4,5,6,7,8].map(w=>({week:w,count:['d1','d2','d3'].filter(d=>state.logs[`w${w}-${d}`]?.completed).length}));
 p.innerHTML=`<div class="hero"><span class="badge">PROGRESS</span><h2>${completed} workouts completed</h2><p class="muted">Strength, consistency, measurements, and recovery trends in one place.</p></div>
 <div class="stat-grid"><div class="mini-stat"><span>Core completion</span><strong>${consistency.reduce((a,x)=>a+x.count,0)} / 24</strong></div><div class="mini-stat"><span>Current week</span><strong>${consistency[state.week-1].count} / 3</strong></div><div class="mini-stat"><span>Logged volume</span><strong>${volumes.reduce((a,x)=>a+x.value,0).toLocaleString()} ${state.settings.units}</strong></div></div>
 <div class="card"><h3>Weekly consistency</h3><div class="consistency-grid">${consistency.map(x=>`<div class="consistency-week"><span>W${x.week}</span><div>${[1,2,3].map(n=>`<i class="${n<=x.count?'filled':''}"></i>`).join('')}</div></div>`).join('')}</div></div>
 <div class="card"><h3>Training volume trend</h3>${svgLineChart(volumes,'Training volume trend')}</div>
 <div class="card"><h3>Hand mobility trend</h3>${svgLineChart(mobility,'Right hand mobility scores')}</div>
 <div class="card"><h3>Hand pain trend</h3>${svgLineChart(pain,'Right hand pain scores')}<p class="muted small">A rising pain trend is a signal to reduce loading and consult your clinician—not something to push through.</p></div>
 <div class="card"><h3>Add body measurement</h3><div class="metrics"><label>Date<input id="measureDate" type="date" value="${new Date().toISOString().slice(0,10)}"></label><label>Body weight<input id="bodyWeight" inputmode="decimal" placeholder="Optional"></label><label>Chest<input id="chestMeasure" inputmode="decimal" placeholder="Optional"></label><label>Hips / glutes<input id="hipMeasure" inputmode="decimal" placeholder="Optional"></label></div><button class="button" id="saveMeasure">Save measurement</button></div>
 <div class="card"><h3>Measurement history</h3>${state.bodyLogs.length?state.bodyLogs.slice().reverse().map(x=>`<p><strong>${x.date}</strong> · Weight ${x.weight||'—'} · Chest ${x.chest||'—'} · Hips ${x.hips||'—'}</p>`).join(''):'<p class="muted">No measurements yet.</p>'}</div>`;
...
function renderSettings(){
 const p=document.getElementById('settings');p.innerHTML=`<div class="hero"><span class="badge">SETTINGS</span><h2>Install, personalize, and protect your data</h2><p class="muted">The app works offline and stores results locally. Export a backup before changing devices.</p></div>
 <div class="card"><h3>Workout preferences</h3><div class="metrics"><label>Default rest timer<select id="restSeconds"><option value="60">60 seconds</option><option value="90">90 seconds</option><option value="120">120 seconds</option><option value="180">180 seconds</option></select></label><label>Weight units<select id="units"><option value="lb">Pounds (lb)</option><option value="kg">Kilograms (kg)</option></select></label></div><button class="button" id="savePrefs">Save preferences</button></div>
 <div class="card"><h3>Install on your phone</h3><p class="muted">Once hosted on GitHub Pages, install it from your browser so it opens like a standalone app and remains available offline.</p><button class="button" id="settingsInstall">Install app</button></div>
 <div class="card"><h3>Backup results</h3><div class="actions"><button class="button" id="exportData">Export results</button><label class="button secondary">Import results<input id="importData" type="file" accept="application/json" hidden></label><button class="button danger" id="resetData">Reset all data</button></div></div>
 <div class="card"><h3>Cloud sync</h3><div class="sync-status"><span>SUPABASE READY</span><strong>Credentials required</strong></div><p class="muted">The interface is ready for cloud sync, but a Supabase project URL, public anon key, authentication, and database table must be created before live cross-device saving can be enabled. Do not use a service-role key in this front end.</p></div>
 <div class="card"><h3>GitHub Pages deployment</h3><p>Upload these files to a repository, open <strong>Settings → Pages</strong>, choose <strong>Deploy from a branch</strong>, and select the main branch root.</p></div>`;
 restSeconds.value=String(state.settings.restSeconds);units.value=state.settings.units;
 document.getElementById('savePrefs').onclick=()=>{state.settings.restSeconds=Number(restSeconds.value);state.settings.units=units.value;save();alert('Preferences saved.')};
 const install=async()=>{if(deferredInstallPrompt){deferredInstallPrompt.prompt();await deferredInstallPrompt.userChoice;deferredInstallPrompt=null}else alert('Use your browser menu and choose “Add to Home Screen” or “Install app.”')};document.getElementById('settingsInstall').onclick=install;
